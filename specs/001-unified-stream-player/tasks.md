...
- [ ] [T026] [US3] [P1] **PRIORITY** Implement YouTube client (`src/network/YouTubeClient.h`, `src/network/YouTubeClient.cpp`)
  ...
  <!-- DROPPED: YouTube client task removed per product decision (2025-11-15). -->
  <!-- If/when YouTube support is reconsidered, create a new backlog task rather than re-opening T026. -->
...
- [X] [T027.4] [US3] [P1] Make FFmpeg decoder resilient to small initial buffers (avoid blocking in `startDecoding`)
...
### Completed: 60/63 (95%)
### Remaining: 3 tasks
### Moved to Spec 002: 0 test tasks (unit-test tasks completed)
...
### Remaining / Deprecated Tasks

#### Deprecated (marked)
- **T026** [US3] [P1] - YouTube client (yt-dlp subprocess integration) — DEPRECATED
  - Rationale: no plan to support YouTube in the short term; integration via `yt-dlp` is intentionally deferred.
- **T037** [US5] [P3] - YouTube search — DEPRECATED
  - Rationale: same as T026; dependent functionality is deferred.
- **T052-T055** [Polish] - Performance optimization & error handling (4 tasks) — DEPRECATED
  - Rationale: benchmarking and deep performance tuning are long-term efforts; keep these as backlog items for a future sprint.

#### Active / Pending
1. **T056** [Polish] - Internationalization infrastructure (setup) — COMPLETED
   - Goal: Add baseline i18n infrastructure, extract user-facing strings, and prepare resource files for translation.
   - Subtasks:
     - Add a translation resource loader and `tr()` usage guidance for Qt/C++ strings.
     - Create `i18n/` directory under `resource/lang/` and add an example `en.json`/`zh.json` skeleton.
     - Add a small build-time check / documentation note describing how to extract strings (`xgettext`/Qt Linguist workflow).
     - Add unit-test that verifies the translation loader can load `en` and `zh` skeletons.
  - Acceptance criteria:
    - Application can load language packs from `resource/lang/<locale>.json` (or QTranslator equivalents).
    - Developer doc explains how to add new locales and regenerate language files.

2. **T057** [Polish] - Internationalization: Translate strings (follow-up) — PENDING
   - Note: T057 will be scheduled after T056 completes; this is the actual translation effort and requires stakeholder input.

### Unit-test tasks (Spec 002) — Completed
- [X] **T040** [Test] - AudioSource unit tests (implemented, test utilities added)
- [X] **T041** [Test] - Playlist unit tests (implemented, TempWorkspace isolation)
- [X] **T047** [Test] - AudioBuffer unit tests (implemented)
- [X] **T050** [Test] - Bilibili integration test (added integration guards and fallbacks)
- [X] **T051** [Test] - Playlist playback integration test (implemented)
